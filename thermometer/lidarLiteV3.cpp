...
lidarLiteV3::~lidarLiteV3()
{
	std::cout << "lidarLiteV3 destructor called" << std::endl;
}

bool _dbg;

// Initialize wiring I2C interface to LidarLite
int lidarLiteV3::lidar_init(bool dbg) {
    int fd;
    _dbg = dbg;
...
int lidarLiteV3::lidar_read(int fd) {
    int hiVal, loVal, i = 0;
...
unsigned char lidarLiteV3::lidar_version(int fd) {
    return((unsigned char)_read_byteNZ(fd, VERSION_REG));
}

unsigned char lidarLiteV3::lidar_status(int fd) {
    return((unsigned char)wiringPiI2CReadReg8(fd, STATUS_REG));
}


void lidarLiteV3::lidar_status_print(unsigned char status) {
    if (status != 0)
